...
$Select  = array('ID', 'NAME', 'IBLOCK_ID', 'DEPTH_LEVEL', 'PICTURE', 'DESCRIPTION', 'CODE', 'DETAIL_PICTURE', 'UF_*');
$rsSections = CIBlockSection::GetList(array("SORT"=>"ASC"), $arFilter, false, $Select);
while ($arSection = $rsSections->Fetch())
{
    PR($arSection);
}*/

$VARIABLES_SECTION = array();
$res = CIBlockSection::GetByID($arResult['VARIABLES']['SECTION_ID']);
if ($ar_res = $res->GetNext()) {
    $arResult['DATA_SECTION'] = $ar_res;
    $arResult['DATA_SECTION']['PICTURE'] = CFile::GetPath($ar_res["PICTURE"]);
    $arResult['DATA_SECTION']['DETAIL_PICTURE'] = CFile::GetPath($ar_res["DETAIL_PICTURE"]);
}
...
<? if ($arResult['DATA_SECTION']['PICTURE']): ?>
    <!-- desktop - 1920x150px, mobile - 126x480 -->
    <div class="category-banner js_category-banner"

         data-mobile-background="<?= $arResult['DATA_SECTION']['PICTURE'] ?>"
         data-desktop-background="<?= $arResult['DATA_SECTION']['DETAIL_PICTURE'] ?>">
...
<? if ($arResult['DATA_SECTION']['DESCRIPTION']): ?>
    <hr>
    <div class="promo">
        <div class="container">
            <?= $arResult['DATA_SECTION']['DESCRIPTION'] ?>
        </div>
    </div>
<? endif; ?>
