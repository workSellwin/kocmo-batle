...
<? if (!empty($arResult['ITEM_ROWS'])): ?>
    ...
    <div class="swiper-container scroll-slider js_scroll-slider">
        <div class="swiper-wrapper scroll-slider__wrapper" data-item-id="<?=$arParams['TabsSliderKey']?>">
...
<script>
    $(document).ready(function(){
        if('<?=$arParams['TabsSliderKey']?>' != ''){
            var i = 0;
            $(".panels div[data-item-id=<?=$arParams['TabsSliderKey']?>] div.products-item").each(function(){
                i++;
            });
            if(i <=0 ){
                $('.tabs span[data-id=<?=$arParams["TabsSliderKey"]?>]').remove();
                $('.panels div[data-id=<?=$arParams["TabsSliderKey"]?>]').remove();
            }
        }
    });
</script>