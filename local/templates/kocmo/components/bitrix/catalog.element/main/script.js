$(document).ready(function () {
    ...
    $('#prod_form').submit(function () {
        ...
        var this_id = $('input[name=OFFERS_ID]').val();
...
function orrers($this) {
    var this_id = $($this).attr('id');
    var this_offers = DATA_OFFERS[this_id];


    $('#prod_form input[name=OFFERS_ID]').val(this_offers['ID']);

    //встовляем цену офирса
    if ( this_offers['PRICE']['DISCOUNT'] > 0) {
        $('.product__old-price').show();
        $('.product__old-price div').text(this_offers['PRICE']['BASE_PRICE']);
        $('.product__price div').text(this_offers['PRICE']['PRICE']);
        $('.product__price').show();
        $('.products-item__label').text('-'+this_offers['PRICE']['PERCENT']+'%');
    } else {
        $('.product__old-price').hide();
        $('.product__price').show();
        $('.product__price div').text(this_offers['PRICE']['BASE_PRICE']);
    }

    //переопределение input ADD_PROD_BASKET
    if (this_offers['QUANTITY'] <= 0){
        $('#prod_form input[name=ADD_PROD_BASKET]').val('N');
    } else {
        $('#prod_form input[name=ADD_PROD_BASKET]').val('Y');
    }
    ...
    if (this_offers['QUANTITY'] > 0 && this_offers['PRICE']['PRICE'] > 0) {
    ...
    if(this_offers['ADD_BASKET'] == 'N' && this_offers['QUANTITY'] > 0 && this_offers['PRICE']['PRICE'] > 0){
        $('div.product__counter').show();
    }else{
        $('div.product__counter').hide();
    }
...
function clickCounterButton($this) {
...
function keyupCounterButton($this) {
    var this_id = $($this).attr('data-offers-id');
    var this_offers = DATA_OFFERS[this_id];
