...
if ($arResult['MODULES']['catalog']) {
    ...
    $arParams['OFFER_TREE_PROPS'] = ['COLOR_REF_2'];
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])) {
    ...
    foreach ($arResult['OFFERS'] as &$arOffer) {
        foreach ($arSKUPropIDs as &$strOneCode) {
            if (isset($arOffer['PROPERTIES'][$strOneCode])) {
                ...
                $arSKUPropList[$strOneCode]['PROPERTY_TYPE'] == Iblock\PropertyTable::TYPE_LIST
                    ? $arOffer['PROPERTIES'][$strOneCode]['VALUE_ENUM_ID']
                    : $arOffer['PROPERTIES'][$strOneCode]['VALUE']
                );
    ...
    foreach ($arResult['OFFERS'] as $keyOffer => $arOffer) {
        ...
        foreach ($arSKUPropIDs as $propkey => $strOneCode) {
            ...
            if (isset($arOffer['PROPERTIES'][$strOneCode])) {
                ...
                if ('directory' == $arSKUPropList[$strOneCode]['USER_TYPE']) {
                    $intValue = $arSKUPropList[$strOneCode]['XML_MAP'][$arOffer['PROPERTIES'][$strOneCode]['VALUE']];
                    $arCell['VALUE'] = $intValue;
                } elseif ('L' == $arSKUPropList[$strOneCode]['PROPERTY_TYPE']) {
                    $arCell['VALUE'] = (int)$arOffer['PROPERTIES'][$strOneCode]['VALUE_ENUM_ID'];
                } elseif ('E' == $arSKUPropList[$strOneCode]['PROPERTY_TYPE']) {
                    $arCell['VALUE'] = (int)$arOffer['PROPERTIES'][$strOneCode]['VALUE'];
                }
        ...
        if (CIBlockPriceTools::clearProperties($arOffer['PROPERTIES'], $arParams['OFFER_TREE_PROPS'])) {
    ...
    foreach ($arResult['OFFERS'] as $keyOffer => $arOffer) {
        ...
        if (!empty($arOffer['PROPERTIES'])) {
            $boolSKUDisplayProps = true;
            $arSKUProps = array();
            foreach ($arOffer['PROPERTIES'] as &$arOneProp) {
...
if (!empty($arResult['PROPERTIES'])) {
    foreach ($arResult['PROPERTIES'] as $propKey => $arDispProp) {
        if ('F' == $arDispProp['PROPERTY_TYPE'])
            unset($arResult['PROPERTIES'][$propKey]);
    }
}

$arResult['SKU_PROPS'] = $arSKUPropList;
$arResult['DEFAULT_PICTURE'] = $arEmptyPreview;
//PR($arResult['SKU_PROPS']);
...
if ($arResult['DETAIL_PICTURE']['ID']):
    $arResult['RES']['PHOTO_BIG'][] = CFile::ResizeImageGet($arResult['DETAIL_PICTURE']['ID'], array('width' => 545, 'height' => 360), BX_RESIZE_IMAGE_PROPORTIONAL, true);
endif;
if ($arResult['DETAIL_PICTURE']['ID']):
...
$arResult['RES']['PANET_TEXT'] = [
    'product-description' => [
        'ACTIVE' => $arResult['DETAIL_TEXT'] ? 'Y' : 'N',
        'NAME' => 'описание',
