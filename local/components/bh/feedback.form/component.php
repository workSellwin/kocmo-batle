...
if (!empty($arParams["ALX_NAME_LINK"])) {
    if (!((substr($arParams["ALX_NAME_LINK"], 0, 1) == '#') || (substr($arParams["ALX_NAME_LINK"], 0, 1) == '.'))) {
        $arResult["ALX_NAME_LINK"] = "#form_id_" . $ALX . ".alx_feedback_popup";
    } else {
        $arResult["ALX_NAME_LINK"] = $arParams["ALX_NAME_LINK"];
    }
} else {
    $arResult["ALX_NAME_LINK"] = "#form_id_" . $ALX . ".alx_feedback_popup";
}
...
if (is_array($_FILES["myFile"]["name"])) {
    foreach ($_FILES["myFile"]["name"] as $k => $value) {
        ...
        if (!is_array($_FILES["myFile"]["name"][$k])) {
            ...
            if ((in_array($arFileName[count($arFileName) - 1],
                        $arParamTypeFileTrim) || empty($arTypeFile[$code]["FILE_TYPE"])) && !empty($_FILES["myFile"]["tmp_name"][$k])) {
                $arFile = Array();
            ...
            foreach ($_FILES["myFile"]["name"][$k] as $mk => $mval) {
                ...
                if ((in_array($arFileName[count($arFileName) - 1],
                            $arParamTypeFileTrim) || empty($arTypeFile[$code]["FILE_TYPE"])) && !empty($_FILES["myFile"]["tmp_name"][$k][$mk])) {
                    $arFile = Array();
                    $_FILES["myFile"]["name"][$k][$mk] = CUtil::translit($_FILES["myFile"]["name"][$k][$mk], "ru",
                        $params);
                    ...
                    $errorFile[$codeID] .= GetMessage("ALX_FIELD1") . $arTypeFile[$code]["NAME"] . '". ' . GetMessage("DISABLE_FILE");
                        ...
                        $arParams["PROPERTY_FIELDS_REQUIRED"]) && $_FILES["myFile"]["error"][$k][$mk] == 4) {
                    $errorFile[$codeID] .= GetMessage("ALX_FIELD1") . $arTypeFile[$code]["NAME"] . '". ' . GetMessage("EMPTY_FILE");
                }
            }
        }
...
// send form
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["FEEDBACK_FORM_" . $ALX] && $arParams["SECTION_FIELDS_ENABLE"] == "Y" && $_POST["REFRESH"] == "Y") {
    ...
    if ($_POST["type_question_" . $ALX] != '') {
        $_POST["FIELDS"]["CATEGORY_" . $ALX] = $_POST["type_question_" . $ALX];
    }
        ...
        if (!$ID = $el->Add($arElementFields)) {
            ...
            foreach ($arMessFormProps as $arMFProp) {
                ...
                if ($arMFProp["SYSTEM"]["PROPERTY_TYPE"] == "L") {
                    ...
                    if (is_array($arMFProp["VALUE"]) && !empty($arMFProp["VALUE"])) {
                        ...
                        $arMessForm[$arMFProp["SYSTEM"]["ID"]] .= "\n" . "//" . $_SERVER["SERVER_NAME"] . CFile::GetPath($arMFProp["SYSTEM"]["VALUE"]) . "\n";
                    }
            ...
            if ($arParams["SHOW_MESSAGE_LINK"] == "Y") {
                $strEditUrl = GetMessage("ALX_CP_IBLOCK_ELEMENT_EDIT",
                    Array(
                        "#ID#" => $ID,
                        "#IBLOCK_TYPE#" => trim(htmlspecialcharsEx($arParams["IBLOCK_TYPE"])),
                        "#LID#" => LANGUAGE_ID,
            ...
            if ($arParams["ADD_LEAD"] == "Y") {
                $arLeadFields = array();
                $arLeadFields['SOURCE_ID'] = (!empty($arParams["LEAD_SOURCE_ID"]) ? $arParams["LEAD_SOURCE_ID"] : "WEB");
                foreach ($arParams as $key => $value) {
                    if (stripos($key, 'LEAD_') === 0 && $value) {
                        $k = substr($key, 5);

                        if ($arPropsIds[$value]["PROPERTY_TYPE"] == "F") {
                            if (!empty($arMessFormProps[$arPropsIds[$value]["ID"]]["VALUE"])) {
                                $strValue = "";
                                if (is_array($arMessFormProps[$arPropsIds[$value]["ID"]]["VALUE"])) {
                                    $strValue = implode(" ", $arMessFormProps[$arPropsIds[$value]["ID"]]["VALUE"]);
                                } else {
                                    $strValue = $arMessFormProps[$arPropsIds[$value]["ID"]]["VALUE"];
                                }

                                $arLeadFields[$k] = $strValue;
                            }
                        } else {
                            $arLeadFields[$k] = $PROPS[$value];
                        }
                    }
                }

                $arLeadFields['COMMENTS'] = ($arParams["FB_TEXT_SOURCE"] == "DETAIL_TEXT" ? $arElementFields['DETAIL_TEXT'] : $arElementFields['PREVIEW_TEXT']);
...
if ($obCache->InitCache(3600, serialize(array($arPFiltr, "section")), $cachePath)) {
    ...
    $rsSect = CIBlockSection::GetList(Array("SORT" => "ASC"), $arFilter, false,
        array("ID", "NAME", "CODE", "IBLOCK_SECTION_ID", "DEPTH_LEVEL", "LEFT_MARGIN", "RIGHT_MARGIN"));
...
if (!empty($arTSect)) {
    ...
    if (in_array("CATEGORY_" . $ALX, $arParams["PROPERTY_FIELDS_REQUIRED"])) {
        $arResult["TYPE_QUESTION_REQUIRED"] = "Y";
    }
}
...
if (!empty($_POST["TARGET_ELEMENT_ID"])) {
    $target = intval($_POST["TARGET_ELEMENT_ID"]);
    $res = CIBlockElement::GetList([], ["ID" => "$target", "CHECK_PERMISSIONS" => "Y"], false, false,
        ["IBLOCK_ID", "ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "DETAIL_PICTURE"]);
        ...
        if (empty($ob["PREVIEW_PICTURE"])) {
            if (!empty($ob["DETAIL_PICTURE"])) {
                $arResult["POPUP_PICTURE"] = CFile::ResizeImageGet($ob["DETAIL_PICTURE"],
                    Array("width" => 290, "height" => 180), BX_RESIZE_IMAGE_PROPORTIONAL);
            }
        } else {
            $arResult["POPUP_PICTURE"] = CFile::ResizeImageGet($ob["PREVIEW_PICTURE"],
                Array("width" => 290, "height" => 180), BX_RESIZE_IMAGE_PROPORTIONAL);
        }
    } else {
        die();
    ...
    $arResult["ELEMENT_NAME"] = $ob["NAME"];
}

$this->IncludeComponentTemplate();


if (count($arResult["TYPE_QUESTION"]) >= 1 || array_walk_recursive($arResult["FIELDS"], function ($v, $k) {
        if ($k === 'TYPE' && ($v === 'L' || $v === 'G')) {
            return true;
        }
    })) {
